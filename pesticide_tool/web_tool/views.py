...
def pesticide_search(request, template='pesticide_search.html'):
  ...
  return render_to_response(template, {'pesticides_typeahead': simplejson.dumps(pesticides_typeahead), 'active_ingredients_typeahead': simplejson.dumps(active_ingredients_typeahead)}, context_instance=RequestContext(request))
...
def brand_page(request, brand_name, template='brand_page.html'):
  ...
  if logger:
    logger.debug("Begin brand_page search: %s(%s) Path: %s" % (search_term, brand_name, request.path))

  brand_json = [];
  try:
    brand_info = Brand.objects.filter(name__iexact=search_term).all()[:1].get()
    #Even though we are retreiving a single brand, the template is setup to handle
    #multiple results. We put this result into an array for the template to use.
